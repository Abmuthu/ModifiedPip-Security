...
class TempFile(object):
  ...
  def _default_temporary_directory(self, prefix):
    ...
    try:
      ...
      raise ssl_commons__exceptions.Error(err)
  ...
  def __init__(self, prefix='tuf_temp_'):
    ...
    <Exceptions>
      ssl_commons__exceptions.Error on failure to load temp dir.
    ...
    if temp_dir is not None and ssl_crypto__formats.PATH_SCHEMA.matches(temp_dir):
      try:
        self.temporary_file = tempfile.NamedTemporaryFile(prefix=prefix,
                                                          dir=temp_dir)
  ...
  def read(self, size=None):
    ...
    <Exceptions>
      ssl_commons__exceptions.FormatError: if 'size' is invalid.
    ...
    if size is None:
      ...
      if not (isinstance(size, int) and size > 0):
        raise ssl_commons__exceptions.FormatError
  ...
  def decompress_temp_file_object(self, compression):
    ...
    <Exceptions>
      ssl_commons__exceptions.FormatError: If 'compression' is improperly formatted.

      ssl_commons__exceptions.Error: If an invalid compression is given.

      ssl_commons__exceptions.DecompressionError: If the compression failed for any reason.
    ...
    # Raise 'ssl_commons__exceptions.FormatError' if there is a mismatch.
    ssl_crypto__formats.NAME_SCHEMA.check_match(compression)
    
    if self._orig_file is not None:
      raise ssl_commons__exceptions.Error('Can only set compression on a TempFile once.')

    if compression != 'gzip':
      raise ssl_commons__exceptions.Error('Only gzip compression is supported.')
    ...
    try:
      ...
      raise ssl_commons__exceptions.DecompressionError(exception)
...
def get_file_details(filepath, hash_algorithms=['sha256']):
  ...
  <Exceptions>
    ssl_commons__exceptions.FormatError: If hash of the file does not match HASHDICT_SCHEMA.

    ssl_commons__exceptions.Error: If 'filepath' does not exist. 
  ...
  # 'ssl_commons__exceptions.FormatError' is raised on incorrect format.
  ssl_crypto__formats.PATH_SCHEMA.check_match(filepath)
  ssl_crypto__formats.HASHALGORITHMS_SCHEMA.check_match(hash_algorithms)
  ...
  if not os.path.exists(filepath):
    raise ssl_commons__exceptions.Error('Path ' + repr(filepath) + ' doest not exist.')
  ...
  for algorithm in hash_algorithms:
    digest_object = ssl_crypto__hash.digest_filename(filepath, algorithm)
  ...
  # Raise 'ssl_commons__exceptions.FormatError' if there is a mismatch.
  ssl_crypto__formats.HASHDICT_SCHEMA.check_match(file_hashes)
...
def ensure_parent_dir(filename):
  ...
  <Exceptions>
    ssl_commons__exceptions.FormatError: If 'filename' is improperly formatted.
  ...
  # Raise 'ssl_commons__exceptions.FormatError' on a mismatch.
  ssl_crypto__formats.PATH_SCHEMA.check_match(filename)
...
def file_in_confined_directories(filepath, confined_directories):
  ...
  <Exceptions>
   ssl_commons__exceptions.FormatError: On incorrect format of the input.
  ...
  # Raise 'ssl_commons__exceptions.FormatError' if there is a mismatch.
  ssl_crypto__formats.RELPATH_SCHEMA.check_match(filepath)
  ssl_crypto__formats.RELPATHS_SCHEMA.check_match(confined_directories)
...
def find_delegated_role(roles, delegated_role):
  ...
  <Exceptions>
    ssl_commons__exceptions.RepositoryError, if the list of roles has invalid data.
  ...
  # Raise 'ssl_commons__exceptions.FormatError' if any are improperly formatted.
  ssl_crypto__formats.ROLELIST_SCHEMA.check_match(roles)
  ssl_crypto__formats.ROLENAME_SCHEMA.check_match(delegated_role)
  ...
  for index in six.moves.xrange(len(roles)):
    ...
    if name is None:
      ...
      raise ssl_commons__exceptions.RepositoryError(no_name_message)
    ...
    # Does this role have the same name?
      ...
      if name == delegated_role:
        ...
        # ...there are at least two roles with the same name.
          ...
          raise ssl_commons__exceptions.RepositoryError(duplicate_role_message)
...
def ensure_all_targets_allowed(rolename, list_of_targets, parent_delegations):
  ...
  <Exceptions>
    ...
    ssl_commons__exceptions.ForbiddenTargetError:
    ...
    ssl_commons__exceptions.RepositoryError:
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_commons__exceptions.FormatError' if any are improperly formatted.
  ssl_crypto__formats.ROLENAME_SCHEMA.check_match(rolename)
  ssl_crypto__formats.RELPATHS_SCHEMA.check_match(list_of_targets)
  ssl_crypto__formats.DELEGATIONS_SCHEMA.check_match(parent_delegations)
  ...
  if role_index is not None:
    ...
    if allowed_child_path_hash_prefixes is not None:
      ...
      if not consistent(actual_child_targets,
        ...
        raise ssl_commons__exceptions.ForbiddenTargetError(message)
      ...
      for child_target in actual_child_targets:
        for allowed_child_path in allowed_child_paths:
          ...
          raise ssl_commons__exceptions.ForbiddenTargetError('Role '+repr(rolename)+' specifies'+\
      ...
      raise ssl_commons__exceptions.FormatError(repr(role) + ' did not contain one of ' +\
  ...
  # 'rolename' child role.
    ...
    raise ssl_commons__exceptions.RepositoryError('The parent role has not delegated to '+\
                              repr(rolename)+'.')
...
def paths_are_consistent_with_hash_prefixes(paths, path_hash_prefixes):
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_commons__exceptions.FormatError' if any are improperly formatted.
  ssl_crypto__formats.RELPATHS_SCHEMA.check_match(paths)
  ssl_crypto__formats.PATH_HASH_PREFIXES_SCHEMA.check_match(path_hash_prefixes)
...
def get_target_hash(target_filepath):
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_commons__exceptions.FormatError' if there is a mismatch.
  ssl_crypto__formats.RELPATH_SCHEMA.check_match(target_filepath)

  # Calculate the hash of the filepath to determine which bin to find the 
  # target.  The client currently assumes the repository uses
  # 'HASH_FUNCTION' to generate hashes and 'utf-8'.
  digest_object = ssl_crypto__hash.digest(HASH_FUNCTION)
...
def load_json_string(data):
  ...
  <Exceptions>
    ssl_commons__exceptions.Error, if 'data' cannot be deserialized to a Python object.
  ...
  try:
    ...
    raise ssl_commons__exceptions.Error(message)
...
def load_json_file(filepath):
  ...
  <Exceptions>
    ssl_commons__exceptions.FormatError: If 'filepath' is improperly formatted.

    ssl_commons__exceptions.Error: If 'filepath' cannot be deserialized to a Python object.
...
def digests_are_equal(digest1, digest2):
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_commons__exceptions.FormatError' if there is a mismatch.
  ssl_crypto__formats.HEX_SCHEMA.check_match(digest1)
  ssl_crypto__formats.HEX_SCHEMA.check_match(digest2)
